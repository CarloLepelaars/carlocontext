---
description: Only for Claude-based agentic commerce using anthropics/commerce-agents. Build an eval suite for a shopping or merchant agent, with a runner, the first ten cases against the user's own catalog, and a replay gate for CI. Use when an agent has a working flow and no measurements.
