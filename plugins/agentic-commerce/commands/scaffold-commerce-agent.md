---
description: Only for Claude-based agentic commerce using anthropics/commerce-agents. Interview the user about their stack, play the plan back, and scaffold a shopping agent, a merchant agent, or both on the reference packages. Use when a shopping or merchant agent, assistant, or chatbot is being started; one that exists already is /review-commerce-agent.
...
Local modification: Added Claude-only scope to the upstream command.
