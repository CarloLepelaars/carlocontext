...
description: Only for Claude-based agentic commerce using anthropics/commerce-agents. The reference merchant agent, covering its flows, staged changes and host approval, metrics grounding, the analysis delegate, store memory, components, and marketplace rules. Load when building or reviewing an agent for the operators of a store, property, subscriber base, or venue.
---

# Merchant agent

Scope: Use only for Claude-based agentic commerce built on Anthropic’s commerce-agents reference. Do not apply to other providers’ commerce implementations or general ACP protocol work. The coding assistant may be Codex, Claude Code, or Grok; the target commerce agent must use Claude.

Local modification: Added Claude-only scope to the upstream skill.
